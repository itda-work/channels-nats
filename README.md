...
- **mailbox는 마지막 `receive()`가 취소될 때 사라진다.** 컨슈머가 끊기면 대기 중이던 `receive()`가 취소되고, 그것이 채널의 끝을 알 수 있는 유일한 신호다. 그때 mailbox와 (일반 채널이면) 그 구독을 정리한다. 다만 컨슈머가 **채널 레이어 메시지를 받고 그대로 종료하면**(`StopConsumer`) 대기 중인 `receive()`가 없어 그 mailbox 하나는 프로세스가 살아 있는 동안 남는다.
...
- **직접 `send`와 `group_send` 사이에는 순서가 없다.** 각 경로 안에서는 순서가 지켜진다(같은 채널로 `send`를 연달아 하면 보낸 순서대로 온다). 하지만 `send` → `group_send` → `send`를 한 프로세스가 순서대로 불러도 수신은 `[1, 3, 2]`가 된다. 서로 다른 subject이고 nats-py가 구독마다 따로 디스패치하기 때문이다. `channels_redis`는 발행자 쪽에서 그룹을 멤버별 쓰기로 펼치기 때문에 순서가 유지되지만, 그 펼치기를 없앤 것이 이 레이어의 존재 이유다(10,000 멤버 `group_send`가 8.7초에서 3.2 ms로).
