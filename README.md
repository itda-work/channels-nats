...
- **mailbox는 마지막 `receive()`가 취소될 때 사라진다. 단, 그룹에 속한 채널은 남는다.** 컨슈머가 끊기면 대기 중이던 `receive()`가 취소되고, 그것이 채널의 끝을 알 수 있는 유일한 신호다. 하지만 취소는 연결 종료의 증거가 못 된다 — `asyncio.wait_for`로 폴링해도 매번 취소가 난다. 그래서 그룹에 남아 있는 채널은 사용 중으로 보고 유지하고, 멤버십은 `group_discard`와 `flush`로만 뺀다.
- **그 결과 두 가지가 남는다.** 그룹에 속하지 않은 채널을 `wait_for`로 폴링하면 타임아웃마다 mailbox가 사라져, 다음 `receive` 전에 온 메시지를 놓친다. 그리고 컨슈머가 **채널 레이어 메시지를 받고 그대로 종료하면**(`StopConsumer`) 대기 중인 `receive()`가 없어 그 mailbox 하나가 프로세스 수명 동안 남는다.
...
- **그룹 멤버십은 프로세스 안에 있다.** 프로세스가 죽으면 그 멤버십도 사라지므로 `group_expiry`는 형식상 유지된다. 다른 프로세스나 **다른 이벤트 루프**가 만든 채널로 `receive`나 `group_add`를 부르면 `ValueError`다. 그 채널로 `send`하는 것은 물론 된다.
