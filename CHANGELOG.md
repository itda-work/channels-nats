...
- 직접 `send`와 `group_send` 사이에 순서 보장이 없다는 것을 README와 CLAUDE.md에 적는다. 각 경로 안의 순서는 테스트로 고정했다. 두 경로를 합치려면 발행자 쪽에서 그룹을 멤버별 쓰기로 펼쳐야 하는데, 그 펼치기를 없앤 것이 이 레이어의 성능 근거라 고치지 않는다
