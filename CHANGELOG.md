...
- 순서 보장 범위를 **구독 하나 안**으로 정정해 README와 CLAUDE.md에 적는다. 직접 `send` 대 `group_send`뿐 아니라 서로 다른 두 그룹 사이에서도 역전된다. 와이어 순서는 옳고(단일 와일드카드 구독으로 확인) 뒤집는 것은 nats-py의 구독별 대기열·태스크다. 발행자 측 그룹 펼치기가 필요하다는 이전 설명은 틀렸다 — 수신 측 단일 디스패처로 고칠 수 있고, 그것을 지금 감당하지 않기로 한 것이다
